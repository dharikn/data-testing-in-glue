...
def configure_spark(spark: SparkSession, join_repartition: int, checkpoint_dir: Optional[str] = None) -> None:
...
def safe_unpersist(*dfs: DataFrame) -> None:
    for df in dfs:
        try:
            if df is not None: df.unpersist()
        except Exception: pass

def checkpoint_if_enabled(df: DataFrame, enabled: bool, eager: bool=True) -> DataFrame:
